...
export class ProfanityFilter {
  ...
  // FIXED: Enhanced phrase matching that handles phrases containing single banned words
  private matchesPhrase(
    ...
    // ENHANCED: Check for phrases containing individual banned words
    // This handles cases like "fuck person" where "fuck" is a banned word
  ...
  // FIXED: Enhanced word matching for single words
  ...
  // FIXED: Enhanced multilingual detection that handles both directions
  private checkMultilingualPhrases(text: string): boolean {
    // Check for combinations of different language profanity
    let hasProfanity = false;

    // First, check if text contains words from multiple languages
    ...
    if (hasEnglishWords && hasVietnameseChars) {
      // This is potentially multilingual text, check more thoroughly

      // Check each language's words and phrases against the entire text
      for (const config of PROFANITY_CONFIG) {
      ...
      // ENHANCED: Additional check for mixed-language phrases
      // Split text into segments and check each segment
      ...
      // Single language text, but still check against all configs
      // This handles cases where language detection might be incorrect
      for (const config of PROFANITY_CONFIG) {
  ...
  private async performProfanityCheck(
    ...
    try {
      // ENHANCED: Check against all languages, not just detected language
      // This fixes both issues by ensuring comprehensive checking

      // Always check multilingual combinations first (this is the main fix)
      if (this.checkMultilingualPhrases(text)) {
        hasProfanity = true;
        confidence += 0.6; // Higher confidence for multilingual detection
      }

      // Then check specific language patterns
      if (language !== "all") {
        const patterns = this.wordPatterns.get(language) || [];
        ...
        for (const pattern of patterns) {
          ...
          if (matches && matches.some((m) => m && m.trim().length > 0)) {
            hasProfanity = true;
            detectedWords.push(...matches.filter((m) => m && m.trim()));
            confidence += 0.3;
          }
        }
